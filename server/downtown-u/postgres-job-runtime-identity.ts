...
export function assertDowntownUJobRuntimeIdentity(queryable: Queryable): Promise<void> {
  return queryable.query<IdentityRow>(`
    ...
    downtown_relations AS (
      ...
      SELECT pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
        ...
        'relation',pg_catalog.jsonb_build_object(
          ...
          'options',COALESCE((SELECT pg_catalog.jsonb_agg(o.option ORDER BY o.option) FROM pg_catalog.unnest(c.reloptions) o(option)),'[]'::jsonb),
        ...
        'columns',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          ...
          'acl',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
            ...
            ORDER BY CASE WHEN z.grantee=c.relowner THEN 'OWNER' WHEN z.grantee=0 THEN 'PUBLIC' ELSE gr.rolname END,
              CASE WHEN z.grantor=c.relowner THEN 'OWNER' WHEN z.grantor=0 THEN 'PUBLIC' ELSE go.rolname END,z.privilege_type,z.is_grantable),'[]'::jsonb)
        ...
        'constraints',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          'name',x.conname,'type',x.contype,'definition',pg_catalog.pg_get_constraintdef(x.oid,true)) ORDER BY x.conname),'[]'::jsonb)
        ...
        'indexes',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          ...
          'predicate',pg_catalog.pg_get_expr(ix.indpred,ix.indrelid,true)) ORDER BY i.relname),'[]'::jsonb)
        ...
        'inheritance_parents',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          'schema',pn.nspname,'relation',p.relname,'sequence',h.inhseqno) ORDER BY h.inhseqno,pn.nspname,p.relname),'[]'::jsonb)
        ...
        'rules',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          ...
          'definition',pg_catalog.pg_get_ruledef(r.oid,true)) ORDER BY r.rulename),'[]'::jsonb)
        ...
        'policies',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          ...
          'roles',COALESCE((SELECT pg_catalog.jsonb_agg(CASE WHEN role_oid=0 THEN 'PUBLIC' WHEN role_oid=c.relowner THEN 'OWNER' ELSE pr.rolname END
            ORDER BY CASE WHEN role_oid=0 THEN 'PUBLIC' WHEN role_oid=c.relowner THEN 'OWNER' ELSE pr.rolname END)
          ...
          'check',pg_catalog.pg_get_expr(p.polwithcheck,p.polrelid,true)) ORDER BY p.polname),'[]'::jsonb)
        ...
        'triggers',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          'name',t.tgname,'enabled',t.tgenabled,'definition',pg_catalog.pg_get_triggerdef(t.oid,true)) ORDER BY t.tgname),'[]'::jsonb)
          FROM pg_catalog.pg_trigger t WHERE t.tgrelid=c.oid AND NOT t.tgisinternal)
      ) ORDER BY c.relname),'[]'::jsonb)::text,'UTF8')),'hex') hash
      ...
      SELECT pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
        ...
        'acl',(SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
          ...
          ORDER BY CASE WHEN x.grantee=p.proowner THEN 'OWNER' WHEN x.grantee=0 THEN 'PUBLIC' ELSE r.rolname END,x.privilege_type),'[]'::jsonb)
      ...
      ) ORDER BY p.proname,pg_catalog.oidvectortypes(p.proargtypes)),'[]'::jsonb)::text,'UTF8')),'hex') hash FROM downtown_functions p
    ), acl_topology AS (
      SELECT pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(pg_catalog.string_agg(p.proname||'('||pg_catalog.oidvectortypes(p.proargtypes)||'):'||r.rolname||':'||x.privilege_type||':'||x.is_grantable,E'\\n' ORDER BY p.proname,pg_catalog.oidvectortypes(p.proargtypes),r.rolname),'UTF8')),'hex') hash
      FROM downtown_functions p CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(p.proacl,pg_catalog.acldefault('f',p.proowner))) x
      JOIN pg_catalog.pg_roles r ON r.oid=x.grantee WHERE x.grantee<>p.proowner
    )
    SELECT COALESCE((SELECT i.rolcanlogin AND SESSION_USER=CURRENT_USER
      ...
      AND (SELECT hash FROM relation_topology)='82a0c3ffdf9760414cf9443039d6363ada2f3f0d49a71982409bcb7c35740c05'
      AND (SELECT hash FROM function_topology)='29fccd9a7402f282b5f5218fac6a6f16288c40e7e9a5a16c4ab4fcd21fc902b3'
      AND (SELECT hash FROM acl_topology)='d38dffa272d47b85de3b0804daa3bed37ff05968eaa86b806a1760f468a017dd'
